...
class AlertDetailsManager {
    ...
    renderTable(alerts, total) {
        ...
        tbody.innerHTML = alerts.map(alert => {
            ...
            const logSource = alert.log_source ? alert.log_source.toLowerCase() : '';
            ...
            // Source Icon Logic
            let sourceIcon = '<i class="fas fa-question-circle opacity-50"></i>';
            if (logSource === 'linux') {
                sourceIcon = '<i class="fab fa-linux text-white text-lg" title="Linux"></i>';
            } else if (logSource === 'windows') {
                sourceIcon = '<i class="fab fa-windows text-blue-400 text-lg" title="Windows"></i>';
            } else if (logSource) {
                sourceIcon = `<span class="text-xs opacity-60 uppercase">${logSource}</span>`;
            }
    ...
    async exportJSON() {
        console.log("Starting exportJSON...");
        try {
            ...
            // 1. Fetch Data
            // Increasing limit to get more alerts, though backend might cap it.
            const res = await fetch('/api/alerts/?limit=500&offset=0');
            if (!res.ok) throw new Error(`Failed to fetch alerts: ${res.status} ${res.statusText}`);
            const data = await res.json();
            const alerts = data.alerts || [];
            console.log(`Fetched ${alerts.length} alerts for export.`);
            ...
            const btn = document.getElementById('btn-export-json');
            if (btn) {
                btn.innerHTML = '<i class="fas fa-file-export"></i> Export JSON';
                btn.disabled = false;
            }
        }
    }
